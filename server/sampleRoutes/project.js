...
import project from '../sampleModels/project';
...
router.get('/', (req, res) => {
  ...
  if (loginInfo.type == 'professor') {
    return res.json({ 
      result: project.get(classID)
    });
  } else { // 학생일 시 본인꺼만 가져옴
    return res.json({ 
      result: project.get(classID, loginInfo.userid)
    });
  }
...
router.post('/', (req, res) => {
  ...
  return res.json({ 
    result: project.post(classID, title, student, projectID, loginInfo.name)
  });
...
router.put('/', (req, res) => {
  ...
  if (status === "start") {
    ...
    return res.send({ result: project.insert(projectID) });
    ...
    return res.send({ result: project.destroy(projectID) });
